...
namespace AtomIoc.Lifestyles
{
    ...
    public class DisposalScope : IDisposalScope
    {
        ...
        public object AddDisposable(object disposable)
        {
            var current = _entry;
            var entry = new DisposeEntry { DisposeItem = (IDisposable)disposable, Next = current };
        ...
        public object AddDisposable(object disposable, Action<object> cleanupDelegate)
        {
            DisposeEntry entry;
            ...
            {
                entry = new DisposeEntry { DisposeItem = (IDisposable)disposable, Next = current };
            }
            else
            {
                entry = new DisposeEntry
                {
                    DisposeItem = (IDisposable)disposable,
