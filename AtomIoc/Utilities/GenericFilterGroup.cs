...
namespace AtomIoc.Utilities
{
    ...
    public class GenericFilterGroup<T1, T2>
    {
        private ImmutableLinkedList<Func<T1, T2, bool>> _typeFilters;

        /// <summary>
        /// Default Constructor
        /// </summary>
        /// <param name="typeFilters"></param>
        public GenericFilterGroup(params Func<T1, T2, bool>[] typeFilters)
        {
            if (typeFilters == null) throw new ArgumentNullException(nameof(typeFilters));

            _typeFilters = typeFilters.Length == 0 ?
                           ImmutableLinkedList<Func<T1, T2, bool>>.Empty :
                           ImmutableLinkedList.From(typeFilters);
        }
        ...
        /// </summary>
        /// <param name="filter"></param>
        public void Add(Func<T1, T2, bool> filter)
        {
            _typeFilters = _typeFilters.Add(filter);
        }
        ...
        /// <param name="group"></param>
        /// <returns></returns>
        public static implicit operator Func<T1, T2, bool>(GenericFilterGroup<T1,T2> group)
        {
            return group.InternalFilter;
        }
        ...
        /// <returns></returns>
        protected virtual bool InternalFilter(T1 t1Value, T2 t2Value)
        {
            if (UseOr)
            {
                foreach (var typeFilter in _typeFilters)
                {
                    if (typeFilter(t1Value, t2Value))
                    {
                        return true;
                    }
                }

                return _typeFilters == ImmutableLinkedList<Func<T1, T2, bool>>.Empty;
            }
